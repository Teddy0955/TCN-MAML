...
def save_confusion_matrix(y_true, y_pred, file_name, args):
    # Move the CUDA tensor to the CPU using .cpu() before creating the confusion matrix
    ...
    if(args.num_class == 2):
        ...
        target_names = ['Approaching', 'Departing', 'Handshaking','High five',
                        ...
                        'Pointing with the right hand', 'Punching with the left hand', 'Punching with the right hand', 'Pushing',
                        'Steady']
    plot_confusion_matrix(file_name,cnf_matrix, classes=target_names, normalize=True,
                          title='Confusion matrix')
                          

def plot_confusion_matrix(file_name, cm, classes,
                          normalize=True,
                          title='Confusion matrix',
                          cmap=plt.cm.Blues):
    """
    ...
    try:
        ...
        #plt.savefig(args.img_path + args.exp_set + file_name + "_" + str(fold+1) + ".jpg")        
        plt.cla()
        plt.close("all")
